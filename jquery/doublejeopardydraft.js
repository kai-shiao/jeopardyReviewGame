function gameplay(e){
	...
	*/
	function dailyDouble(){
		...
		function proceedToBid(){
			...
			function saveBidValue(){
				...
				$bidValue=parseInt($('#bidBox').find('input').val());
	...
	function answerAndScore(){
		...
		function markAnswer(){
			...
			if ($eventTargetID>=5 && $eventTargetID<10 && $inputValue===$answers[$eventTargetID] && $elapsedTime<=10){ 
				...
				$cluePrize=1600;
			} else if ($eventTargetID>=25 && $eventTargetID<=27 && $inputValue===$answers[$eventTargetID] && $elapsedTime<=10){
				$cluePrize=2000; //Exclude daily double tiles 
			} else if ($eventTargetID>=28 && $inputValue===$answers[$eventTargetID] && $elapsedTime<=10){
				$cluePrize=2000;  
			...
			var $doubleJeopardyTotalPrize=parseInt(localStorage.getItem('doubleJeopardyTotalPrize')); 	//Player's total prize winnings so far
			...
			$clueStatus[$eventTargetID]='TRUE'; 					//Update array data after a clue has been attempted.
			console.log($clueStatus);
			localStorage['clueStatus']=JSON.stringify($clueStatus); //Convert back into a string and save the changes back into localStorage.
			/*
	...
	function checkDoubleJeopardyGameProgress(){
		var $i=5;
		var $count=0;
		console.log($clueStatus.length);
		while ($i<$clueStatus.length){
			var	$currentStatus=$clueStatus[$i];
		...
		if ($count===25){
			$('body').append('<div id="completedMessage"><p>CONGRATULATIONS!</p><p>YOU HAVE FINISHED DOUBLE JEOPARDY!</p><a href="finaljeopardy.html"></a></div>');
		}
	}
	...
	if ($eventTargetID>=0 && $eventTargetID<5){ //Category tiles
		...
		if ($clueStatus[$eventTargetID]==='FALSE'){
			showClue();
			startCountingTime();
...
function welcomeTheContestant(){
	...
	function saveContestantName(){
		...
		if ($contestantName===''){				
			...
			function proceedToRules(){
				...
				var $ruleBox='<div id="ruleBox"><span>X</span><h1>ONE-PLAYER DOUBLE JEOPARDY RULES AND NOTES</h1><ul><li>THERE IS A 10-SECOND';
					$ruleBox+=' TIME LIMIT TO PROVIDE ANSWERS TO CLUES.</li><li>FOR ANSWERS REQUIRING A PERSON\'S NAME, ONLY ENTER THE LAST';
					$ruleBox+=' NAME, NOT THE FULL NAME.</li><li>DO NOT REFRESH THE PAGE WHILE PLAYING AS THE WEB APPLICATION WILL RESET ';
					$ruleBox+='<span>ALL</span> GAME PROGRESS AND DATA AND, AS A RESULT, YOU WILL HAVE TO START FROM SCRATCH.</li></ul></div>';
				$('body').append($ruleBox);
	...
	*/
	...
	while($i<5){
		$clueStatus.push(''); //The first 5 array elements are assigned blanks as the corresponding tiles represent the Jeopardy! categories.
		$i++;
	}
		
	while($i>4 && $i<30){
		$clueStatus.push('FALSE'); //Array elements with index position corresponding to the clue tiles (5-29) are assigned a value of 'FALSE'.
		$i++;
	}
	
	console.log($clueStatus); //Check to see the result of the while-loop.
	
	//HTML5 localStorage can only store strings, so $clueStatus must be formatted into a string.
	localStorage['clueStatus']=JSON.stringify($clueStatus); 
	
	console.log(localStorage.getItem('doubleJeopardyTotalPrize'));
	console.log(localStorage.getItem('doubleJeopardyCorrect'));
	console.log(localStorage.getItem('doubleJeopardyAnswered'));
}
